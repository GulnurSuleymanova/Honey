    ...
    <div className="min-h-screen bg-gradient-to-br from-yellow-50 via-amber-50 to-orange-100 p-6 w-full">
      <div className="max-w-full">
        ...
        <div className="bg-white/90 backdrop-blur-sm rounded-2xl shadow-lg border border-amber-200 overflow-hidden">
          <div className="grid grid-cols-3 gap-4 p-6 border-b border-amber-100 bg-gradient-to-r from-yellow-100 to-amber-100 font-semibold text-amber-800">
            <div>Image</div>
            <div>Slug</div>
            <div>Price</div>
          </div>
          ...
          {data?.map((item) => (
            <div
              key={item.id}
              className="grid grid-cols-3 gap-4 p-6 border-b border-amber-100 hover:bg-yellow-50 transition-colors"
            >
